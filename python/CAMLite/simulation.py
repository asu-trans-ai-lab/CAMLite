...
class Simulation:
    def __init__(self, net, demands, signal_manager, args):
        ...
        self.safe_headway_in_simu_interval = 0
                    ...
                    for node_seq_no in mesolink.estimated_cost_tree_for_each_movement[downstream_mesolink_seq_no].keys():
                        ...
                        if node_id in path_length.keys():
                            if path_length[node_id] < mesolink.estimated_cost_tree_for_each_movement[downstream_mesolink_seq_no][node_seq_no]:
                                mesolink.estimated_cost_tree_for_each_movement[downstream_mesolink_seq_no][node_seq_no] = path_length[node_id]
    ...
    def update(self, agent, outgoing_link, t):
        current_node, to_node = outgoing_link.from_node, outgoing_link.to_node
        to_node.available_sim_interval = _MAX_TIME_INTERVAL
        current_node.available_sim_interval = t + self.safe_headway_in_simu_interval
        agent.micro_path_node_seq_no_list.append(to_node.node_seq_no)
        agent.micro_path_link_seq_no_list.append(outgoing_link.link_seq_no)
        agent.micro_path_node_id_list.append(to_node.node_id)
        agent.micro_path_link_id_list.append(outgoing_link.link_id)
    ...
    def startSim(self):
        ...
        self.safe_headway_in_simu_interval = np.round(self.time_headway / self.number_of_seconds_per_interval)
        ...
        for t in range(number_of_simu_intervals):
            ...
            for agent_no in active_agent_list:
                ...
                if agent.m_Veh_LinkDepartureTime_in_simu_interval[-1] == t:
                    ...
                    if agent.next_meso_link_seq_no == -1:
                        outgoing_link_travel = current_node.m_outgoing_link_keep
                        to_node_travel = outgoing_link_travel.to_node

                        outgoing_link_td_travel_time_in_simu_interval = max(round(outgoing_link_travel.length / outgoing_link_travel.td_travel_speed(simu_time_in_min) * 3.6 / self.number_of_seconds_per_interval), 1)
                        if to_node_travel.available_sim_interval > t + outgoing_link_td_travel_time_in_simu_interval:
                            agent.m_Veh_LinkDepartureTime_in_simu_interval[-1] += 1
                        else:
                            self.update(agent, outgoing_link_travel, t)

                    else:
                        current_mesolink = net.meso_link_list[agent.current_meso_link_seq_no]
                        next_mesolink = net.meso_link_list[agent.next_meso_link_seq_no]
                        if current_node in current_mesolink.lane_nodes_dict[next_mesolink]:
                            best_outgoing_link = None
                            min_cost = _MAX_TIME_INTERVAL
                            min_waiting_time = _MAX_TIME_INTERVAL
                            for outgoing_link in current_node.outgoing_link_list:
                                if outgoing_link in inactive_microlinks:
                                    continue
                                if outgoing_link.to_node not in current_mesolink.lane_nodes_dict[next_mesolink]:
                                    continue

                                outgoing_link_td_travel_time_in_simu_interval = max(round(outgoing_link.length / outgoing_link.td_travel_speed(simu_time_in_min) * 3.6 / self.number_of_seconds_per_interval), 1)
                                waiting_time = max(0, outgoing_link.to_node.available_sim_interval - (t + outgoing_link_td_travel_time_in_simu_interval))
                                cost = waiting_time + outgoing_link_td_travel_time_in_simu_interval
                                if cost < min_cost:
                                    min_cost = cost
                                    min_waiting_time = waiting_time
                                    best_outgoing_link = outgoing_link

                            if min_waiting_time > 0:
                                agent.m_Veh_LinkDepartureTime_in_simu_interval[-1] += 1
                            else:
                                self.update(agent, best_outgoing_link, t)
                        else:
                            if agent.next_outgoing_link is not None:
                                outgoing_link_td_travel_time_in_simu_interval = max(round(agent.next_outgoing_link.length / agent.next_outgoing_link.td_travel_speed(simu_time_in_min) * 3.6 / self.number_of_seconds_per_interval), 1)
                                if agent.next_outgoing_link.to_node.available_sim_interval > t + outgoing_link_td_travel_time_in_simu_interval:
                                    agent.m_Veh_LinkDepartureTime_in_simu_interval[-1] += 1
                                else:
                                    self.update(agent, agent.next_outgoing_link, t)
                                    agent.next_outgoing_link = None
                            else:
                                outgoing_link_travel = current_node.m_outgoing_link_keep
                                to_node_travel = outgoing_link_travel.to_node
                                if (to_node_travel not in current_mesolink.accessible_nodes_dict[next_mesolink]) or (random.random() < 0.05):
                                    length_list = [current_mesolink.estimated_cost_tree_for_each_movement[next_mesolink.link_seq_no][outgoing_link.to_node.node_id] + outgoing_link.length for outgoing_link in current_node.m_outgoing_link_change_list]
                                    min_length = min(length_list)
                                    valid_changing_link_list = [current_node.m_outgoing_link_change_list[idx] for idx, length in enumerate(length_list) if length <= min_length + 0.1]

                                    best_outgoing_link_change = None
                                    min_waiting_time = _MAX_TIME_INTERVAL
                                    for outgoing_link in valid_changing_link_list:
                                        outgoing_link_td_travel_time_in_simu_interval = max(round(outgoing_link.length / outgoing_link.td_travel_speed(simu_time_in_min) * 3.6 / self.number_of_seconds_per_interval), 1)
                                        waiting_time = max(0, outgoing_link.to_node.available_sim_interval - (t + outgoing_link_td_travel_time_in_simu_interval))
                                        if waiting_time < min_waiting_time:
                                            best_outgoing_link_change = outgoing_link
                                            min_waiting_time = waiting_time

                                    if min_waiting_time > 0:
                                        agent.next_outgoing_link = best_outgoing_link_change
                                        agent.m_Veh_LinkDepartureTime_in_simu_interval[-1] += 1
                                    else:
                                        self.update(agent, best_outgoing_link_change, t)
